...
import com.codart.animalnicknames.billing.localdb.LocalBillingDb
import com.codart.animalnicknames.billing.localdb.SubState
...
class BillingRepository private constructor(private val application: Application) :
    ...
    val subsSkuDetailsListLiveData: LiveData<List<AugmentedSkuDetails>> by lazy {
        if (::localCacheBillingClient.isInitialized == false) {
            localCacheBillingClient = LocalBillingDb.getDatabase(application)
        }
     ...
     */
    fun startDataSourceConnections(): Boolean {
        Log.d(LOG_TAG, "startDataSourceConnections")
        val c = instantiateAndConnectToPlayBillingService()
        localCacheBillingClient = LocalBillingDb.getDatabase(application)
        return c
    }
    ...
    private fun instantiateAndConnectToPlayBillingService(): Boolean {
    ...
    private fun processPurchases(purchasesResult: Set<Purchase>) =
        CoroutineScope(Job() + Dispatchers.IO).launch {
            ...
            try {
                SubState.saveItems(subIsActive)
            }
            catch (e: Exception)
            {
                Log.e("Save sub error", e.toString())
            }
        }
    ...
    private fun querySkuDetailsAsync(
        ...
        playStoreBillingClient.querySkuDetailsAsync(params) { billingResult, skuDetailsList ->
            when (billingResult.responseCode) {
                BillingClient.BillingResponseCode.OK -> {
                    Log.d(LOG_TAG, "skuDetail list size  ${skuDetailsList!!.size}")
    ...
    fun launchBillingFlow(activity: Activity, skuDetails: SkuDetails) {
        ...
        val purchaseParams = BillingFlowParams.newBuilder().setSkuDetails(skuDetails)
            .setOldSku(oldSku.toString(),"test line 756").build()
    ...
    private object GameSku {
        ...
        val SUBS_SKUS = listOf(SUB_1MONTH, SUB_3MONTH, SUB_6MONTH)
